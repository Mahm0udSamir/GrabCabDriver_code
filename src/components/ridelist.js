            ...
            <TouchableOpacity style={styles.iconClickStyle} onPress={() => this.onPressButton(item, index)}>
                ...
                <View style={styles.flexViewStyle}>
                    ...
                    <View style={styles.textView2}>
                        <Text style={[styles.fareStyle, styles.dateStyle]}>{item.status == 'END' && item.payment_status == 'PAID' ? item.trip_cost > 0 ? "$" + item.trip_cost.toFixed(2) : "$" + parseFloat(item.estimate).toFixed(2) : null}</Text>
                        {
