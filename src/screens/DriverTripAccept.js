    ...
    onPressAccept(item) {
        ...
        dbRef.update(data).then(()=>{
            ...
            this.setState({currentBId:item.bookingId},()=>{
                this.checking();
                this.sendPushNotification(item.customer,item.bookingId,riderData.driver_name+' Accept Your Booking Request')
            })
                ...
                if(mainBookingData.requestedDriver) {      
                    if(mainBookingData.requestedDriver.length == 1) {
                        ...
                        .then(()=>{
                            ...
                            this.sendPushNotification(item.customer,item.bookingId,'Your booking request has rejected by requested driver')
                        })
                    }
                    else {
        ...
        return(
            <View style={styles.mainViewStyle}>
                ...
                <FlatList
                    ...
                    ListEmptyComponent={<View style={{flex:1, justifyContent:"center", alignItems:"center", height: height}}><Text style={styles.addressViewTextStyle}>Currently no riders in your area</Text></View>}
